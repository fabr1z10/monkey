...
def save(file):
    ...
    with open(file, 'wb') as f:
        ...
        while i < len(settings.lines):
            ...
            line_number = int(l[:lc]).to_bytes(2, 'little')
            ...
            #f.write(instruction)
        sorted_instructions = [x[1] for x in sorted(instructions.items())]
...
def openf(file):
    ...
    with open(file, 'rb') as f:
        ...
        while True:
            ...
            m = [str(ln), ' ']
            for b in data:
                m.append(settings.invtoken.get(int(b), chr(b)))
            inst=''.join(m)
            print(inst)
            a0=a1
    ...
    #print('# lines:',settings.LINE_COUNT)
